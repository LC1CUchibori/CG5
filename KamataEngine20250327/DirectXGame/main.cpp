...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	// Shader コンパイル -------------------------------
	// コンパイル済みのShader、エラー時情報格納用の変数
	ID3DBlob* vsBlob = nullptr;          // 頂点シェーダオブジェクト
	ID3DBlob* psBlob = nullptr;          // ピクセルシェーダオブジェクト
	ID3DBlob* shaderErrorBlob = nullptr; // エラーオブジェクト
	...
	hr = D3DCompileFromFile(
		...
		0, &vsBlob, &shaderErrorBlob);
	if (FAILED(hr)) {
		DebugText::GetInstance()->ConsolePrintf(std::system_category().message(hr).c_str());
		if (shaderErrorBlob) {
			DebugText::GetInstance()->ConsolePrintf(reinterpret_cast<char*>(shaderErrorBlob->GetBufferPointer()));
			shaderErrorBlob->Release(); // エラー発生時にリリース
		}
		assert(false);
	}
